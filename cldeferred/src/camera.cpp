...
Camera::Camera() :
    _pitch(0), _yaw(0), _position(0,0,0), _moveSpeed(1.0f),
    _movementFlags(0)
{
    updateViewMatrix();
    setProjection(60.0f, 16.0f/9.0f, 0.01f, 100.0f);
}
...
void Camera::lookAt(QVector3D lookPos)
{
    ...
    const float theta= acos(lookDir.z());
    const float phi= atan2(lookDir.y(), lookDir.x());
...
void Camera::move(float elapsedMsecs)
{
    ...
    const QVector3D moveDir= (front-back) * lookDir + sideStep;

    _position += elapsedMsecs/1e6d * _moveSpeed * moveDir.normalized();
